function [xVect] = psdCompletion_SG(x,K,clique)
...
epsilon = 1.0e-10; % this is only a starting value, will be updated if needed
            ...
            if ~isfield(clique{kk},'NoCliqueInForest')
                ...
                for i=1:clique{kk}.NoC-1
                    ...
                    if ~isempty(edgeSet)
                        ...
                        N_try = 30;
                        try_count = 1;
                        while ~try_flag && try_count<=N_try
                            try
                                LMat = chol(XMat(U,U)+eps*speye(nDim,nDim));
                                try_flag = 1;
                            catch
                                eps = 10*eps;
                                try_count = try_count+1;
                                if try_count>N_try
                                    disp('try_count>N_try')
                                    return
                                end
                            end
                        end
