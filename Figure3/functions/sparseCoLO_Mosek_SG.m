function [x,y,infoCoLO,cliqueDomain,cliqueRange,LOP] = sparseCoLO_Mosek_SG(A,b,c,K,J,parCoLO)
% Modified to include Mosek as a solver, in which case set:
%   parCoLO.SDPsolver = 'mosek';
%   parCoLO.domain = 1; parCoLO.range = 2; parCoLO.EQorLMI = 1;
% Zeyu Kuang, Aug 25 2021 
...
%         [LOP,cliqueD] = dConvCliqueTree(A,sPatternVect,b,c,K,J);
        ...
        [LOP,cliqueD] = dConvCliqueTree_SG(A,sPatternVect,b,c,K,J);  %%% SG edit
%%
...
% -------------------------------------------------------------------------
% -------------------------------------------------------------------------
% ------------------- Zeyu's modification starts here ---------------------
% -------------------------------------------------------------------------
% -------------------------------------------------------------------------
% -------------------------------------------------------------------------

elseif strcmp(parCoLO.SDPsolver,'mosek')
    
    % skip parameter setting for Mosek
    
    if isfield(LOP.J,'f') && ~isempty(LOP.J.f) && (LOP.J.f == size(LOP.A,1))
        infoCoLO.CPUsolver = toc(tStart);
        prob = convert_sedumi2mosek(LOP.A, LOP.b, LOP.c, LOP.K);
        
        
        
                %%%%%%%%%% setting parameters
        %%%%%%%%%% (https://docs.mosek.com/latest/toolbox/parameters.html)
        
        % Controls how the problem is scaled before the interior-point optimizer is used.
%         param.MSK_IPAR_INTPNT_SCALING = 'MSK_SCALING_AGGRESSIVE'; %default "FREE"; "NONE", "MODERATE", "AGGRESSIVE"
        
        % Controls how much effort is used in scaling the problem before a simplex optimizer is used.
%        param.MSK_IPAR_SIM_SCALING = 'MSK_SCALING_AGGRESSIVE'; %default "FREE"; "NONE", "MODERATE", "AGGRESSIVE"

        % Controls whether the primal or the dual problem is solved.
%          param.MSK_IPAR_INTPNT_SOLVE_FORM = 'MSK_SOLVE_DUAL'; %default FREE; PRIMAL, DUAL (see MSKsolveforme)

        % Controls whether the primal or the dual problem is solved by the primal-/dual-simplex optimizer.
%         MSK_IPAR_SIM_SOLVE_FORM = 'FREE'; %default 'FREE'

        % Controls whether the presolve is applied to a problem before it is optimized.
%         MSK_IPAR_PRESOLVE_USE = 'FREE'; %default 'FREE'
...
%         [r, res] = mosekopt('minimize echo(0)', prob); %echo(0) is for supress printing on screen
        ...
        % extract optimal variables x and y
        [x, y] = sol_mosek2sedumi(res, LOP.K.s);
        
        infoCoLO.CPUsolver = toc(tStart) - infoCoLO.CPUsolver;
        infoCoLO.SDPsolver = [];
    elseif isfield(LOP.K,'f') && ~isempty(LOP.K.f) && (LOP.K.f == size(LOP.A,2))
        % skip dual formulation for Mosek
    else
        fprintf('LOP to be solved is neither equality nor LMI standard form.\n');
    end    
...
if strcmp(parCoLO.SDPsolver,'sedumi') || strcmp(parCoLO.SDPsolver,'sdpa') || strcmp(parCoLO.SDPsolver,'sdpt3') || strcmp(parCoLO.SDPsolver,'mosek')
