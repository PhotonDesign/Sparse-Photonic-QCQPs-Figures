...
% This code computes *derivatives of THICKNESSES* with respect to the reflection and 
...
%
% Outputs: size Nw x Nt x 1 x N, where N is # of layer thicknesses

function[R,dR,r,dr,T,dT,t,dt] = multilayer_film_derivs_thickness(layerMaterials, layerThicknesses, w, theta, pol)
...
for p = 1:length(pol)
    ...
    dMdt = dMMatrix_thickness(layerMaterials, layerThicknesses, w, beta, pol(p), sz);
    dM11 = dMdt(:,:,1,:);
    dM21 = dMdt(:,:,3,:);
    ...
    if (nargout>4)
        ...
        % If t and dt is purely about As/A0, not about sqrt(kxs/kx0)*As/A0,
        % otherwise, comment next two lines. If incident material is the
        % same with exit material, then it doesn't matter whether or not
        % you comment next two lines since kx0=kxs.
        t{p}=t{p}./sqrt(kxs./kx0);
        dt{p}=dt{p}./sqrt(kxs./kx0);
